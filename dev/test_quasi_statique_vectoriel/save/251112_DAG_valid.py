...
from typing import Any, Dict, List, Tuple, Literal, Union, Optional, Mapping
...
class SolverDAG:
    ...
    def _build_objects(cfg: Cfg,
        ...
        for c in cfg.converters:
            converters[c.id] = ConstantEtaConverter(
                id=c.id, from_bus=c.from_bus, to_bus=c.to_bus, eta=float(c.eta),
            )
